...
import firebase from './firebase'
 ...
 class SendProps extends PureComponent {
    ...
    constructor(props) {
      ...
      /*  let jeetvar=firebase.database().ref().child('clinic').orderByKey();
       // console.log(jeetvar+"  jeetvar")
        jeetvar.on("value",snapshot=>{
         snapshot.forEach(child =>{
           console.log(child.key);  //will print key of clinic
...
//pass dateString and offsetTime to SlotBooking
  render() {
    ...
    return (
        (!this.state.isData1Available && !this.state.isData2Available) && 
      <div>
        <SlotBooking doctorName={this.props.doctorName} slotsDatabase={this.slotsDatabase} 
            ...
            dateString={this.dateString} offsetTime={this.offsetTime}
        />
      </div>
    )
  }
